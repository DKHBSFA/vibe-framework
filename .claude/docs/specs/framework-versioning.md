...
## Cosa
Aggiungere un sistema di versioning al framework, cosicché gli utenti che eseguono `vibe-framework.sh` su altri progetti possano sapere:
...
Dopo ogni install/update, `vibe-framework.sh` scrive la versione installata in `.claude/.framework-version` nel progetto target.
...
| `vibe-framework.sh` | Modificare: leggere version, mostrarla, confrontarla, scriverla |
| `.claude/.framework-version` | **Nuovo** (generato dallo script nel target) |

### Verifica
- `./vibe-framework.sh /tmp/test-project` → mostra versione nell'header
